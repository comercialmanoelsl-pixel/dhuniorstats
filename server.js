...
app.get("/api/finished-games", async (req,res)=>{
  ...
  for(const date of dates){
    if(SPORTMONKS_KEY){
      const smData = await safe(getAllSportMonksByDate(date));
      if(smData.__error) errors.push({source:"SportMonks",date,error:smData.__error});
      else normalized.push(...smData.map(normalizeSportMonks).filter(g=>g.state==="post"));
    }

    if(API_FOOTBALL_KEY){
      const apiData = await safe(api(`/fixtures?date=${date}`));
      if(apiData.__error) errors.push({source:"API-Football",date,error:apiData.__error});
      else normalized.push(...(apiData.response||[]).map(normalizeApiFootball).filter(g=>g.state==="post"));
    }

    await Promise.all(ESPN_LEAGUES.map(async ([key,name])=>{
      const data = await safe(espn(`${key}/scoreboard?dates=${yyyymmdd(date)}&limit=200`));
      if(!data.__error) normalized.push(...(data.events||[]).map(ev=>normalizeEspn(ev,name)).filter(g=>g.state==="post"));
    }));
  }

  const games = dedupeGamesV56(normalized, dates[0]).sort((a,b)=>{
    const da = new Date(a.date||0).getTime();
    const db = new Date(b.date||0).getTime();
    return db-da || String(a.league).localeCompare(String(b.league));
  });

  res.json({ok:true,mode:"finished-recent",days,total:games.length,games,errors});
});
