...
function normalizeEspnGame(event, league){
  ...
  return {
    ...
    leagueId:"", season:"", leagueKey:league.key, league:league.name, date:event.date || "",
    ...
    state:type.state || "", live:type.state==="in", minute:event.status?.displayClock || "",
...
function normalizeApiGame(m){
  ...
  return {
    ...
    leagueId:l.id || "", season:l.season || "", league:l.name || "", leagueKey:"", date:f.date || "",
...
function standingOf(standings, teamId){
...
function makePregame(game, ctx){
  ...
  const confidence = evidence >= 4 ? "Alta" : evidence >= 2 ? "Moderada" : "Baixa";
  ...
  return {
    ...
    homeContextScore: Math.round(homeScore),
    awayContextScore: Math.round(awayScore),
    finalizationTrend: canAssert ? `${leader} aparece com melhor tendência pré-jogo` : "Dados insuficientes",
    cornerTrend: canAssert ? "Precisa de média real de escanteios por equipe para afirmar com segurança" : "Dados insuficientes",
...
function makeLive(stats){
  ...
  return {
    ...
    base:[
      `Finalizações: ${finalHome} x ${finalAway}.`,
      `Chutes no gol: ${targetHome} x ${targetAway}.`,
      `Escanteios: ${cornerHome} x ${cornerAway}.`,
      `Posse: ${possHome || "-"}% x ${possAway || "-"}%.`
    ]
  };
}
  ...
  try{
    ...
    if(fixtureId){
      const [statsJson, eventsJson, lineupsJson] = await Promise.all([
        ...
        safeApi(`/fixtures/lineups?fixture=${fixtureId}`)
      ]);
    ...
    if(game?.home?.id && game?.away?.id){
      ...
      const [hr, ar, hhr, aar, h2h, standings, hs, as] = await Promise.all([
      ...
      pregame = makePregame(game, {
        ...
        homeSeasonStats:hs, awaySeasonStats:as
      });
    } else {
      pregame = {
        hasEnoughData:false, confidence:"Baixa", evidenceCount:0, pressureTeam:"",
        finalizationTrend:"Dados insuficientes", cornerTrend:"Dados insuficientes",
        reading:"Esse jogo não possui IDs suficientes da API-Football para montar contexto real.",
        base:["Sem IDs de time/competição vindos da API-Football."]
      };
    }
    ...
    const analysis = {
      live, pregame,
      contextSource:"API-Football: últimos jogos, casa/fora, H2H, tabela e estatísticas de temporada",
      main: live.hasEnoughData ? live : pregame
    };
    res.json({ok:true,fixtureId:fixtureId||"",teamStats,events,lineups,analysis});
  }catch(e){
    res.status(500).json({ok:false,error:String(e.message||e)});
  }
});

app.listen(PORT, () => console.log("DhuniorStats V10 Contexto Real rodando na porta " + PORT));
