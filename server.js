...
app.get("/api/game-details", async (req,res)=>{
  try{
    ...
    if(SPORTMONKS_KEY&&sportmonksId){
      const data=await safeSm(`/fixtures/${sportmonksId}?include=statistics.type;participants;events.type;events.participant;lineups.player;lineups.position;lineups.type;scores`);
      if(data.data){
...
app.get("/api/standings", async (req,res)=>{
  try{
    ...
    if(!leagueId || !season) return res.json({ok:false,error:"leagueId/season ausente",standings:[]});
    if(typeof safeApi === "function"){
      const data = await safeApi(`/standings?league=${leagueId}&season=${season}`);
      return res.json({ok:true,standings:data.response?.[0]?.league?.standings?.[0]||[]});
    }
    return res.json({ok:false,error:"API-Football não disponível",standings:[]});
  }catch(e){res.status(500).json({ok:false,error:String(e.message||e),standings:[]})}
});

app.get("/api/team-profile", async (req,res)=>{
  try{
    const teamId=req.query.teamId, leagueId=req.query.leagueId, season=req.query.season;
    const out={recent:[],stats:null,players:[]};
    if(typeof safeApi === "function" && teamId){
      const [recent, stats, players] = await Promise.all([
        safeApi(`/fixtures?team=${teamId}&last=10`),
        leagueId&&season?safeApi(`/teams/statistics?team=${teamId}&league=${leagueId}&season=${season}`):Promise.resolve({response:null}),
        leagueId&&season?safeApi(`/players?team=${teamId}&league=${leagueId}&season=${season}`):Promise.resolve({response:[]})
      ]);
      out.recent=recent.response||[];
      out.stats=stats.response||null;
      out.players=players.response||[];
    }
