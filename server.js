...
app.get("/api/health", (req,res) => {
  res.json({ ok:true, hasApiFootballKey: !!API_FOOTBALL_KEY });
});
...
app.get("/api/games", async (req, res) => {
  ...
  for (const g of out) {
    ...
    if (!old) {
      ...
      if (g.source === "API-Football") {
        map.set(key, { ...old, ...g, espnId: old.espnId || g.espnId, leagueKey: old.leagueKey || g.leagueKey });
      }
    }
  }
...
function normalizeStats(statResponse) {
  ...
  (statResponse || []).forEach(teamBlock => {
    ...
    (teamBlock.statistics || []).forEach(s => rows.push({
      team,
      label: s.type || "",
      value: s.value ?? ""
    }));
...
function normalizeLineups(lineups) {
  ...
  (lineups || []).forEach(teamBlock => {
    const team = teamBlock.team?.name || "";
    const formation = teamBlock.formation || "";
    (teamBlock.startXI || []).forEach(x => rows.push({
      team,
      formation,
    ...
    (teamBlock.substitutes || []).forEach(x => rows.push({
      team,
      formation,
      player: x.player?.name || "",
      number: x.player?.number || "",
...
function getStat(stats, labels) {
  const item = stats.find(s => labels.some(label => String(s.label).toLowerCase().includes(label)));
  return item ? item.value : "";
}

function splitByTeam(stats) {
  const teams = [...new Set(stats.map(s => s.team).filter(Boolean))];
  const home = teams[0] || "Casa";
  const away = teams[1] || "Fora";
  const by = { home, away, homeStats: {}, awayStats: {} };
  stats.forEach(s => {
    const target = s.team === home ? by.homeStats : by.awayStats;
    target[String(s.label).toLowerCase()] = s.value;
  });
  return by;
}

function findByLabel(obj, labels) {
  const key = Object.keys(obj).find(k => labels.some(l => k.includes(l)));
  return key ? obj[key] : "";
}

function makeAnalysis(stats, events, game) {
  const t = splitByTeam(stats);
  const hs = t.homeStats;
  const as = t.awayStats;

  const homeShots = num(findByLabel(hs, ["total shots", "shots total"]));
  const awayShots = num(findByLabel(as, ["total shots", "shots total"]));
  const homeTarget = num(findByLabel(hs, ["shots on goal", "shots on target"]));
  const awayTarget = num(findByLabel(as, ["shots on goal", "shots on target"]));
  const homeCorners = num(findByLabel(hs, ["corner"]));
  const awayCorners = num(findByLabel(as, ["corner"]));
  const homePoss = num(findByLabel(hs, ["ball possession", "possession"]));
  const awayPoss = num(findByLabel(as, ["ball possession", "possession"]));

  let homeScore = num(game?.home?.score);
  let awayScore = num(game?.away?.score);
  let homePressure = 35;
  let awayPressure = 35;

  homePressure += Math.min(homeShots * 2.1, 28);
  homePressure += Math.min(homeTarget * 3.2, 22);
  homePressure += Math.min(homeCorners * 2.6, 22);
  homePressure += Math.min(homePoss * 0.22, 16);
  if (homeScore < awayScore) homePressure += 8;

  awayPressure += Math.min(awayShots * 2.1, 28);
  awayPressure += Math.min(awayTarget * 3.2, 22);
  awayPressure += Math.min(awayCorners * 2.6, 22);
  awayPressure += Math.min(awayPoss * 0.22, 16);
  if (awayScore < homeScore) awayPressure += 8;

  homePressure = Math.max(0, Math.min(100, Math.round(homePressure)));
  awayPressure = Math.max(0, Math.min(100, Math.round(awayPressure)));

  const leader = homePressure >= awayPressure ? t.home : t.away;
  const leaderPressure = Math.max(homePressure, awayPressure);
  const textLevel = leaderPressure >= 80 ? "MUITO ALTA" : leaderPressure >= 65 ? "ALTA" : leaderPressure >= 45 ? "MODERADA" : "BAIXA";

  let trend = "Sem tendência forte detectada";
  const reasons = [];
  ...
  if ((homePoss > awayPoss && leader === t.home) || (awayPoss > homePoss && leader === t.away)) reasons.push("Posse de bola ajuda a sustentar o domínio.");

  if (leaderPressure >= 75) trend = `${leader} pressionando forte e criando mais perigo.`;
  else if (leaderPressure >= 55) trend = `${leader} tem leve superioridade ofensiva.`;

  const cornerTrend = homeCorners + awayCorners >= 8 ? "Alta" : homeCorners + awayCorners >= 4 ? "Moderada" : "Baixa";
  const shotTrend = homeShots + awayShots >= 18 ? "Alta" : homeShots + awayShots >= 10 ? "Moderada" : "Baixa";
...
app.get("/api/game-details", async (req, res) => {
  ...
  try {
    if (!fixtureId) throw new Error("Esse jogo não veio com fixtureId da API-Football. Tente outro jogo da lista ou atualize.");
    ...
    const analysis = makeAnalysis(teamStats, events, gameJson);
