  ...
  try{
    ...
    if(SPORTMONKS_KEY && sportmonksId){
      ...
      if(!fx.__error && fx.data){
        ...
        lineups=getSportMonksLineups(fx.data);
      }
    }
    ...
    if((!stats || !stats.available || !events.length || !lineups.length) && API_FOOTBALL_KEY && apiFootballId){
      ...
      if(!lineups.length && !lu.__error){
        for(const team of lu.response||[]){
          for(const p of team.startXI||[]) lineups.push({team:team.team?.name||"", type:"Titular", name:p.player?.name||"", number:p.player?.number||"", pos:p.player?.pos||""});
          for(const p of team.substitutes||[]) lineups.push({team:team.team?.name||"", type:"Banco", name:p.player?.name||"", number:p.player?.number||"", pos:p.player?.pos||""});
        }
      }
    }
...
app.get("/api/standings", async (req,res)=>{
  try{
    ...
    if(data.__error) return res.json({ok:false,standings:[],error:data.__error});
