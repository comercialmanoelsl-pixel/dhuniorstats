...
app.get("/api/live-games", async (req,res)=>{
  ...
  if(API_FOOTBALL_KEY){
    const d=await safe(api(`/fixtures?live=all`));
    if(d.__error) errors.push({source:"API-Football",error:d.__error});
    else normalized.push(...(d.response||[]).map(normalizeApiFootball));
  }

  if(SPORTMONKS_KEY){
    // SportMonks doesn't always provide one global live endpoint in every plan, so use selected date and filter live.
    const d=await safe(getAllSportMonksByDate(date));
    if(d.__error) errors.push({source:"SportMonks",error:d.__error});
    else normalized.push(...d.map(normalizeSportMonks).filter(g=>g.live));
  }

  for(const [key,name] of ESPN_LEAGUES){
    const ep=await safe(espn(`${key}/scoreboard?dates=${yyyymmdd(date)}&limit=200`));
    if(!ep.__error) normalized.push(...(ep.events||[]).map(ev=>normalizeEspn(ev,name)).filter(g=>g.live));
  }
  ...
  const games=[...map.values()].filter(g=>g.live).sort((a,b)=>String(a.league).localeCompare(String(b.league)));
