...
function renderLineups(){
  ...
  if(!raw.length){
    $("mainPanel").innerHTML=`<div class="title">Escalações</div>
      <div class="dataNotice">Escalação oficial ainda não disponível. O site atualizará automaticamente quando a API liberar.</div>`;
    return;
  }
  ...
  let homeAll=raw.filter(p=>belongs(p,homeName));
  let awayAll=raw.filter(p=>belongs(p,awayName));

  if(!homeAll.length && !awayAll.length){
    const half=Math.ceil(raw.length/2);
    homeAll=raw.slice(0,half);
    awayAll=raw.slice(half);
  }
  ...
  let homeStarters=homeAll.filter(starter);
  let awayStarters=awayAll.filter(starter);

  if(!homeStarters.length) homeStarters=homeAll.slice(0,11);
  if(!awayStarters.length) awayStarters=awayAll.slice(0,11);

  function unique(list){
    const seen=new Set();
    return list.filter(p=>{
      const k=`${p.number||""}-${normName(p.name)}`;
  ...
  homeStarters=unique(homeStarters).slice(0,11);
  awayStarters=unique(awayStarters).slice(0,11);
  ...
  const scheme=formations[formation]||formations["4-3-3"];
  ...
  function nodes(players,side){
    ...
    scheme.forEach((count,lineIndex)=>{
      const progress = lineIndex/(scheme.length-1 || 1);
      const x = side==="home" ? 8 + progress*34 : 92 - progress*34;
      ...
      for(let i=0;i<count;i++){
        ...
        out.push(`
          <div class="playerNode" style="left:${x}%;top:${y}%">
            <div class="playerCircle">${esc(p.number||idx)}</div>
  ...
  $("mainPanel").innerHTML=`
    ...
    <div class="lineupMeta">
      <span class="realBadge">Oficial via API</span>
      <span class="formationPill">${esc(formation)}</span>
...
function renderSideLineups(){
  ...
  const group=(title,list)=>`<div class="sideLineupGroup">
    ...
    ${list.slice(0,14).map(p=>`<div class="sidePlayer"><div class="sideNum">${esc(p.number||"-")}</div><div><div class="sideName">${esc(short(p.name))}</div><div class="sideSub">${esc(p.pos||"")} · ${esc(p.type||"")}</div></div></div>`).join("")}
  </div>`;
