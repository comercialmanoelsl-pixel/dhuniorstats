...
$("refreshBtn").onclick=()=>activeLeagueSlug?loadLeagueGames(activeLeagueSlug):loadGames();$("searchInput").oninput=renderMatches;$("searchInput").onkeydown=e=>{if(e.key==="Enter")searchGames()};$("searchBtn").onclick=searchGames;$("filterInput").onchange=()=>{$("filterInput").value==="post"?loadFinishedGamesV59():renderMatches()};$("dateInput").onchange=()=>activeLeagueSlug?loadLeagueGames(activeLeagueSlug):loadGames();
...
function renderLineups(){
  ...
  if(!home.starters.length && !away.starters.length){return `<div class="title">Escalações</div><div class="dh60Empty"><b>Escalação ainda não liberada.</b><span>Quando a SportMonks/API retornar lineup, ela aparece aqui sem inventar dados.</span></div>`}
  const pitch=(name,team)=>`<section class="dh60LineupCard"><header><b>${esc(name)}</b><span>${esc(modeLabel)}</span></header><div class="dh60PitchOne">${dh60Nodes(team.starters)}</div>${dh60Bench('Reservas · '+name,team.bench)}</section>`;
  return `<div class="title">Escalações</div><div class="dh60LineupWrap">${pitch(homeName,home)}${pitch(awayName,away)}</div>`;
}
