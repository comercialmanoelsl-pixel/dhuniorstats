...
const matchFullCache={}; let matchFullData=null; let smComponentData=null; let coverageData=null; let allGames=[], selected=null, details=null, tab="overview", sideTab="ia", quick="", homeMode=true, detailTimer=null, activeLeagueSlug="";
...
async function loadDetails(g){
  ...
  try{
    ...
    try{
      const full = await fetchMatchFullForGame(g);
      if(full?.ok){
        applyMatchFullToDetails(full);
        smComponentData = full;
      }
    }catch(_){ matchFullData=null; smComponentData=null; }
    try{
