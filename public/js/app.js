...
function renderLineups(){
  ...
  // If source has bad/missing team name, split safely without claiming certainty.
  let fallbackSplit=false;
  ...
  // IMPORTANT: formation only if API explicitly sends it.
  ...
  $("mainPanel").innerHTML=`
    ...
    <div class="lineupMeta">
      ...
      ${fallbackSplit?'<span class="formationPill">times separados por fallback</span>':''}
    ...
    <div class="dataNotice" style="margin-top:10px">
      A posição no campo é organizada por função do jogador quando a API não informa a formação oficial. 
      O DhuniorStats não inventa esquema tático.
    </div>
