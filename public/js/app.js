...
let coverageData=null; let allGames=[], selected=null, details=null, tab="overview", sideTab="ia", quick="", homeMode=true, detailTimer=null, activeLeagueSlug="";
...
function renderLineups(){
  ...
  let homeAll=unique(raw.filter(p=>belongs(p,homeName)));
  let awayAll=unique(raw.filter(p=>belongs(p,awayName)));
...
function renderMainPanel(){
  ...
  else if(tab==="stats")$("mainPanel").innerHTML=`<div class="title">Estatísticas</div>${statRows(details.stats)}`;
