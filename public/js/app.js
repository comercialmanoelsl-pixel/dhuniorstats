...
function renderLineupsV47Hard(){
  ...
  if(groups){
    homePlayers = groups.home?.all || [];
    awayPlayers = groups.away?.all || [];
    unknownPlayers = groups.unknown || [];
  } else {
    homePlayers = raw.filter(p=>p.side==="home" || sameTeamNameV47(p.team, selected?.home?.name));
    awayPlayers = raw.filter(p=>p.side==="away" || sameTeamNameV47(p.team, selected?.away?.name));
    unknownPlayers = raw.filter(p=>!homePlayers.includes(p) && !awayPlayers.includes(p));
  }

  let warning = "";
  if(!homePlayers.length && !awayPlayers.length && unknownPlayers.length){
    const split = splitUnknownLineupV47(unknownPlayers);
    homePlayers = split.home;
    awayPlayers = split.away;
    warning = `<div class="dataNotice warnV47">A SportMonks retornou jogadores, mas sem identificação 100% segura de time nesta resposta. Para não misturar tudo, o DhuniorStats separou visualmente em duas colunas e manteve o aviso de segurança.</div>`;
  }

  if(!homePlayers.length && !awayPlayers.length){
    return `<div class="dataNotice">Escalação oficial/provável ainda não retornada pela API para esta partida.</div>
      <button class="smallBtn" onclick="openDiagnosticForSelected()">Diagnosticar esta partida</button>`;
  }

  return `<div class="lineupV47Header">
      <b>Escalações</b>
      <span>Visual seguro: mandante à esquerda, visitante à direita</span>
    </div>
    ${warning}
    <div class="lineupV46 v47">
      ${renderTeamLineupV47(selected?.home?.name || "Mandante", "esquerda", homePlayers)}
      ${renderTeamLineupV47(selected?.away?.name || "Visitante", "direita", awayPlayers)}
    </div>`;
}
...
const matchFullCache={}; const matchCenterCache={}; let matchCenterData=null; let matchFullData=null; let smComponentData=null; let coverageData=null; let allGames=[], selected=null, details=null, tab="overview", sideTab="ia", quick="", homeMode=true, detailTimer=null, activeLeagueSlug="";
...
async function loadDetails(g){
  ...
  try{
    ...
    renderScoreboard(selected);renderMainPanel();renderSide();scheduleAutoRefreshV54();
  }catch(e){$("mainPanel").innerHTML=emptyStateV54("Não foi possível carregar",""+(e.message||e),"⚠️")}
}
