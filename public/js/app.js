...
let allGames=[], selected=null, details=null, tab="overview", sideTab="ia", quick="", homeMode=true;
...
function showHome(){homeMode=true;$("homeView").style.display="block";$("matchView").style.display="none";renderHome();renderSide()}
...
function renderLineups(){
  ...
  if(!l.length){$("mainPanel").innerHTML=`<div class="title">Escalações</div><div class="dataNotice">Escalações reais indisponíveis para esta partida. O DhuniorStats não exibe escalação simulada.</div>`;return}
  const rows=l.map(x=>`<div class="event"><b>${esc(x.pos||"-")}</b><div><b>${esc(x.name)}</b><br><small>${esc(x.team)} · ${esc(x.type)} ${x.number?("#"+x.number):""}</small></div></div>`).join("");
  $("mainPanel").innerHTML=`<div class="title">Escalações reais</div><div class="realBadge">Dados da API</div><br><br>${rows}`;
}
...
function renderSideLineups(){
  const l=details?.lineups||[];
  if(!l.length){$("sideContent").innerHTML=`<div class="title">Escalações</div><div class="dataNotice">Escalações reais indisponíveis.</div>`;return}
  $("sideContent").innerHTML=`<div class="title">Escalações</div>`+l.slice(0,20).map(x=>`<div class="event"><b>${esc(x.pos||"-")}</b><div><b>${esc(x.name)}</b><br><small>${esc(x.team)} · ${esc(x.type)}</small></div></div>`).join("");
}
...
function openMatch(g){homeMode=false;$("homeView").style.display="none";$("matchView").style.display="block";loadDetails(g)}
