...
let allGames=[], selected=null, details=null, tab="overview", sideTab="ia", quick="", homeMode=true, detailTimer=null, activeLeagueSlug="";
...
function renderLineups(){
  ...
  if(!l.length){
    $("mainPanel").innerHTML=`<div class="title">Escalações visuais</div>
    <div class="dataNotice">Escalação oficial ainda não disponível. O sistema atualizará automaticamente quando as APIs liberarem os lineups oficiais.</div>`;
    return;
  }

  const byTeam = name => l.filter(x=>{
    const t=String(x.team||"").toLowerCase();
    return t.includes(String(name).toLowerCase()) || String(name).toLowerCase().includes(t);
  });

  let home=byTeam(homeName);
  let away=byTeam(awayName);
  ...
  home = (home.filter(isStarter).length ? home.filter(isStarter) : home.slice(0,11));
  away = (away.filter(isStarter).length ? away.filter(isStarter) : away.slice(0,11));

  const homeBench = byTeam(homeName).filter(x=>!home.includes(x));
  const awayBench = byTeam(awayName).filter(x=>!away.includes(x));
  ...
  const formation = details?.formation || "4-3-3";
  const scheme = rows[formation] || [1,4,3,3];

  function short(n){
    return String(n||"").split(" ").slice(-1)[0].slice(0,12);
  }

  function buildNodes(players,isHome){
    const out=[];
    let idx=0;

    scheme.forEach((count,rowIndex)=>{
      const yBase = isHome
        ? 82 - (rowIndex*(60/(scheme.length-1||1)))
        : 18 + (rowIndex*(60/(scheme.length-1||1)));

      for(let i=0;i<count;i++){
        const p=players[idx++] || {};
        const x = 50 + ((i - (count-1)/2) * (70/count));
        out.push(`
          <div class="playerNode" style="left:${x}%;top:${yBase}%;">
            <div class="playerCircle">${p.number||idx}</div>
            <div class="playerName">${short(p.name||"Jogador")}</div>
          </div>
        `);
      }
    });
      ...
      <div class="benchCard">
        ...
        <div class="benchGrid">
          ${list.slice(0,12).map(p=>`
            <div class="benchItem">
              <b>#${p.number||"-"} ${short(p.name||"Reserva")}</b>
              <small>${p.pos||"Banco"} · ${p.source||"API"}</small>
            </div>
          `).join("") || '<div class="dataNotice">Banco indisponível.</div>'}
        </div>
      </div>
    `;
  }
  ...
  $("mainPanel").innerHTML=`
    <div class="title">Escalações visuais</div>
    <div class="realBadge">Atualização automática · ${formation}</div>
    ...
    <div class="visualPitch">
      ...
      <div class="teamLabel home">${homeName}</div>
      <div class="teamLabel away">${awayName}</div>

      ${buildNodes(home,true)}
      ${buildNodes(away,false)}
    </div>

    <div class="benchWrap">
      ${bench(homeBench, "Banco · " + homeName)}
      ${bench(awayBench, "Banco · " + awayName)}
    </div>
  `;
}
