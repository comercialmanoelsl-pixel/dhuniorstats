...
function canonicalTeamV56Front(name){
  ...
  const aliases = [
    ...
    [/^(junior fc|atletico junior|atletico junior fc|cd popular junior fc sa|junior)$/,'atletico junior'],
    [/^(palmeiras|se palmeiras|sociedade esportiva palmeiras)$/,'palmeiras'],
    [/^(boca juniors|boca juniors fc|ca boca juniors)$/,'boca juniors'],
    [/^(universidad catolica|universidad catolica del ecuador|u catolica)$/,'universidad catolica'],
    [/^(barcelona sc|barcelona guayaquil|barcelona sporting club)$/,'barcelona sc'],
    [/^(independiente medellin|independiente medellin fc|deportivo independiente medellin)$/,'independiente medellin'],
    [/^(deportivo la guaira|dep la guaira|la guaira)$/,'deportivo la guaira']
  ];
...
function dedupeFixturesV56(list=[]){
  ...
  const rank = g => {
    const s = String(g?.source||'').toLowerCase();
    if(s.includes('sportmonks')) return 100;
    if(s.includes('api-football')) return 70;
    return 10;
  };

  const cleanScore = v => (v===null || v===undefined || v==="") ? "" : String(v);
  const betterLeagueName = (a,b)=>{
    const A=String(a||""), B=String(b||"");
    // prefere nome mais completo, mas normaliza Libertadores/Sul-Americana na exibição
    if(/sudamericana/i.test(A)) return "Copa Sudamericana";
    if(/sudamericana/i.test(B)) return "Copa Sudamericana";
    if(/libertadores/i.test(A)) return "Copa Libertadores";
    if(/libertadores/i.test(B)) return "Copa Libertadores";
  ...
  const merge = (old,g)=>{
    ...
    const home = {...old.home, ...g.home};
    const away = {...old.away, ...g.away};
    const finalHomeScore = cleanScore(g?.home?.score) !== "" ? g.home.score : old.home?.score;
    const finalAwayScore = cleanScore(g?.away?.score) !== "" ? g.away.score : old.away?.score;
    return {
      ...old,
      ...g,
      id: old.id || g.id,
      sportmonksId: old.sportmonksId || g.sportmonksId,
      apiFootballId: old.apiFootballId || g.apiFootballId,
      league: betterLeagueName(old.league, g.league),
      home:{...home, name:preferred.home?.name || home.name, score:finalHomeScore},
      away:{...away, name:preferred.away?.name || away.name, score:finalAwayScore}
    };
  };

  for(const g of list){
    if(!g?.home?.name || !g?.away?.name) continue;
    const d = String(g?.date || g?.time || g?.starting_at || '').slice(0,10);
    const l = canonicalLeagueV56Front(g?.league||'');
    const h = canonicalTeamV56Front(g?.home?.name||'');
    const a = canonicalTeamV56Front(g?.away?.name||'');

    // IDs continuam valendo, mas as assinaturas por times pegam duplicatas vindas de endpoints diferentes.
    const candidates = [
      fixtureKeyV56Front(g),
      `sig:${d}:${l}:${h}:${a}`,
      `sig:${d}:${l}:${a}:${h}`,
      `teams:${d}:${h}:${a}`,
      `teams:${d}:${a}:${h}`
...
function renderMatches(){
  const games=dedupeFixturesV56 ? dedupeFixturesV56(dh60FilteredGames()) : dh60FilteredGames();
